...
describe('Button', () => {
  ...
  describe('로딩', () => {
    it('loading이 true일 때 버튼을 비활성화한다', () => {
      render(<Button loading>Submit</Button>)
      expect(screen.getByRole('button')).toBeDisabled()
    })

    it('loading이 true일 때 스피너를 렌더링한다', () => {
      render(<Button loading>Submit</Button>)
      const button = screen.getByRole('button')
    ...
    it('loading이 true일 때 children을 invisible로 숨긴다', () => {
      render(<Button loading>Submit</Button>)
      const button = screen.getByRole('button')
      const childrenWrapper = button.querySelector('span:not(.absolute)')
      expect(childrenWrapper).toHaveClass('invisible')
    })

    it('loading이 true일 때 onClick을 호출하지 않는다', async () => {
      const user = userEvent.setup()
      const handleClick = vi.fn()
    ...
    it('loading이 true일 때 aria-busy 속성을 설정한다', () => {
      render(<Button loading>Submit</Button>)
      expect(screen.getByRole('button')).toHaveAttribute('aria-busy', 'true')
    })

    it('loading이 false일 때 aria-busy 속성을 설정하지 않는다', () => {
      render(<Button>Submit</Button>)
      expect(screen.getByRole('button')).not.toHaveAttribute('aria-busy')
    })

    it('disabled와 loading이 모두 true일 때 버튼을 비활성화한다', () => {
      render(
