# 인기 회사 섹션 기능 명세서

## 📋 기능 개요

검색 횟수가 높은 인기 회사를 랭킹 형태로 제공하는 기능입니다.

### 목적

- 투자자에게 주목받는 회사 정보를 한눈에 제공
- 시장의 관심사를 빠르게 파악
- 메인 페이지 진입 시 핵심 정보 노출

---

## 🎯 요구사항

### 1. 표시 개수

- **PC (≥768px)**: 5개 카드 표시
- **모바일 (<768px)**: 3개 카드 표시

### 2. 레이아웃

- 좌에서 우로 1등부터 순서대로 배치
- 카드 형식으로 가로 정렬
- 위치: 메인 페이지 배너 하단

### 3. 데이터 페칭 전략

- **Server Component에서 프리패치**
- **HydrationBoundary를 통해 클라이언트로 전달**
- 클라이언트에서 React Query로 상태 관리

### 4. 서버 캐싱

- 인기 회사는 자주 변경되지 않는 데이터
- Next.js `fetch` + `next.revalidate` 옵션 활용
- 캐시 유효기간: **5분 (300초)**

---

## 🎨 UI/UX 상세

### PC 버전 (≥768px)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  🔥 인기 회사                                                                    │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐              │
│  │  🥇 1   │  │  🥈 2   │  │  🥉 3   │  │    4    │  │    5    │              │
│  │         │  │         │  │         │  │         │  │         │              │
│  │ 회사명   │  │ 회사명   │  │ 회사명   │  │ 회사명   │  │ 회사명   │              │
│  │         │  │         │  │         │  │         │  │         │              │
│  │ 종목코드 │  │ 종목코드 │  │ 종목코드 │  │ 종목코드 │  │ 종목코드 │              │
│  └─────────┘  └─────────┘  └─────────┘  └─────────┘  └─────────┘              │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### 모바일 버전 (<768px)

```
┌─────────────────────────────────────────┐
│  🔥 인기 회사                            │
├─────────────────────────────────────────┤
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  │
│  │  🥇 1   │  │  🥈 2   │  │  🥉 3   │  │
│  │         │  │         │  │         │  │
│  │ 회사명   │  │ 회사명   │  │ 회사명   │  │
│  │         │  │         │  │         │  │
│  │ 종목코드 │  │ 종목코드 │  │ 종목코드 │  │
│  └─────────┘  └─────────┘  └─────────┘  │
└─────────────────────────────────────────┘
```

### 카드 구성

- **순위 표시**: 1~3위는 메달 아이콘 (🥇🥈🥉), 4~5위는 숫자
- **회사명**: 1줄 말줄임표 처리
- **종목코드**: 6자리 코드
- **카드 클릭**: 회사 상세 페이지 또는 검색 결과로 이동

### 반응형 분기점

- **모바일**: `< 768px` - 3개 카드
- **PC**: `≥ 768px` - 5개 카드

---

## 🔧 기술 구현 방안

### 아키텍처 (FSD)

```
apps/web/
├── app/
│   └── page.tsx                              # 메인 페이지 (인기 회사 프리패치)
│
├── widgets/
│   └── popular-companies-section/
│       ├── index.ts
│       ├── popular-companies-section.tsx     # 메인 위젯
│       └── ui/
│           ├── popular-company-card.tsx      # 인기 회사 카드
│           ├── popular-company-list.tsx      # 카드 리스트 (반응형)
│           └── popular-company-skeleton.tsx  # 로딩 스켈레톤
│
├── entities/
│   └── disclosure/
│       ├── api/
│       │   └── popular-companies/
│       │       ├── client.ts                 # 클라이언트 API 호출 (기존)
│       │       └── server.ts                 # 서버 API 호출 + 캐싱 (기존)
│       ├── model/
│       │   └── types.ts                      # PopularCompany 타입 (기존)
│       └── queries/
│           └── prefetch.ts                   # 프리패치 함수 추가
│
└── shared/
    └── lib/
        └── query-keys.ts                     # stocks.popular 쿼리 키 (기존)
```

### 기존 API 활용

인기 회사 데이터는 이미 구현되어 있습니다:

```typescript
// entities/disclosure/api/popular-companies/server.ts (기존)
export async function getPopularCompaniesFromDB(limit: number = 10)

// entities/disclosure/model/types.ts (기존)
export interface PopularCompany {
  rank: number
  corpName: string
  stockCode: string
  searchCount: number
}
```

### 프리패치 함수 추가

```typescript
// entities/disclosure/queries/prefetch.ts (추가)

/**
 * [서버 컴포넌트용] 인기 회사 데이터를 prefetch합니다
 * @param limit - 조회할 최대 건수 (기본값: 5)
 * @returns Dehydrated state (HydrationBoundary에 전달)
 */
export async function prefetchPopularCompanies(limit: number = 5) {
  const queryClient = new QueryClient({
    defaultOptions: {
      queries: {
        staleTime: 5 * 60 * 1000, // 5분
      },
    },
  })

  await queryClient.prefetchQuery({
    queryKey: queries.stocks.popular.queryKey,
    queryFn: () => getPopularCompaniesFromDB(limit),
    staleTime: 5 * 60 * 1000, // 5분
  })

  return dehydrate(queryClient)
}
```

### 메인 페이지 통합

```typescript
// app/page.tsx
import { HydrationBoundary } from '@tanstack/react-query'
import { prefetchTodayDisclosures, prefetchPopularCompanies } from '@/entities/disclosure/server'
import { PopularCompaniesSection } from '@/widgets/popular-companies-section'

export default async function Home() {
  // 병렬로 프리패치 (개별 실패 허용)
  const [todayResult, popularResult] = await Promise.allSettled([
    prefetchTodayDisclosures('all', 6),
    prefetchPopularCompanies(5),
  ])

  // 성공한 결과만 병합
  const dehydratedState = {
    mutations: [],
    queries: [
      ...(todayResult.status === 'fulfilled' ? todayResult.value.queries : []),
      ...(popularResult.status === 'fulfilled' ? popularResult.value.queries : []),
    ],
  }

  return (
    <HydrationBoundary state={dehydratedState}>
      <main>
        <ServiceBanner />
        <PopularCompaniesSection />
        <TodayDisclosures />
      </main>
    </HydrationBoundary>
  )
}
```

### 클라이언트 훅

기존 `usePopularCompanies` 훅을 활용합니다:

```typescript
// entities/disclosure/queries/hooks.ts (기존)
export function usePopularCompanies(limit = 10) {
  return useSuspenseQuery({
    queryKey: queries.stocks.popular.queryKey,
    queryFn: () => getPopularCompanies(limit),
    staleTime: 5 * 60 * 1000, // 5분
  })
}
```

---

## 🚀 구현 단계

### Phase 1: 프리패치 추가

1. **프리패치 함수 추가**
   - `prefetchPopularCompanies` 함수 구현
   - `entities/disclosure/server.ts`에 export 추가

2. **메인 페이지 통합**
   - `Promise.allSettled`로 병렬 프리패치
   - 상태 병합

### Phase 2: UI 컴포넌트

1. **카드 컴포넌트**
   - `popular-company-card.tsx`
   - 순위 표시 (메달/숫자)

2. **리스트 컴포넌트**
   - `popular-company-list.tsx`
   - 반응형 개수 조절 (3개/5개)
   - 가로 정렬

3. **스켈레톤**
   - `popular-company-skeleton.tsx`

### Phase 3: 메인 위젯 통합

1. **위젯 조합**
   - `popular-companies-section.tsx`
   - Suspense + ErrorBoundary 처리

2. **메인 페이지 배치**
   - 배너 하단에 위치
   - 기존 `TodayDisclosures`와 조화

---

## 📊 성능 목표

- **서버 캐시 히트율**: 95% 이상
- **초기 로딩**: 프리패치로 즉시 렌더링
- **캐시 갱신 주기**: 5분

---

## ✅ 체크리스트

- [ ] 프리패치 함수 추가 (`prefetchPopularCompanies`)
- [ ] 서버 export 추가
- [ ] 메인 페이지 프리패치 통합
- [ ] 카드 컴포넌트 (`popular-company-card.tsx`)
- [ ] 리스트 컴포넌트 (`popular-company-list.tsx`)
- [ ] 스켈레톤 컴포넌트
- [ ] 위젯 통합 (`popular-companies-section.tsx`)
- [ ] 반응형 테스트 (PC 5개 / 모바일 3개)

---

## 📝 참고 자료

- [Next.js Data Fetching (fetch + revalidate)](https://nextjs.org/docs/app/building-your-application/data-fetching)
- [TanStack Query Hydration](https://tanstack.com/query/latest/docs/framework/react/guides/ssr)
- [오늘의 공시 명세서](./today-disclosures.md)
